...
check_cidr() {
	# Check if the input is a valid CIDR notation (e.g., 10.8.0.0/24)
	CIDR_REGEX='^(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])/([0-9]|[12][0-9]|3[0-2])$'
	printf '%s' "$1" | tr -d '\n' | grep -Eq "$CIDR_REGEX"
}

parse_subnet() {
	# Parse CIDR notation and extract network components
	# Input: CIDR like "10.8.0.0/24"
	# Sets: vpn_subnet, vpn_network, vpn_netmask, vpn_gateway_ip, vpn_base_ip
	local cidr="$1"
	if ! check_cidr "$cidr"; then
		exiterr "Invalid CIDR notation: $cidr. Expected format: x.x.x.x/xx"
	fi
	
	vpn_subnet="$cidr"
	vpn_network=$(echo "$cidr" | cut -d '/' -f 1)
	vpn_netmask=$(echo "$cidr" | cut -d '/' -f 2)
	
	# Validate netmask is /24 (required for this script)
	if [ "$vpn_netmask" != "24" ]; then
		exiterr "Only /24 subnet mask is supported. Got: /$vpn_netmask"
	fi
	
	# Extract first three octets as base IP
	vpn_base_ip=$(echo "$vpn_network" | cut -d '.' -f 1-3)
	# Gateway is typically .1 in the subnet
	vpn_gateway_ip="${vpn_base_ip}.1"
}

read_subnet_from_conf() {
	# Read VPN subnet from existing wg0.conf file
	# Sets: vpn_subnet, vpn_network, vpn_netmask, vpn_gateway_ip, vpn_base_ip
	if [ ! -f "$WG_CONF" ]; then
		exiterr "WireGuard configuration file not found: $WG_CONF"
	fi
	
	# First, try to read from # SUBNET comment line if it exists
	local subnet_line=$(grep "^# SUBNET " "$WG_CONF" | head -1 | cut -d ' ' -f 3)
	if [ -n "$subnet_line" ] && check_cidr "$subnet_line"; then
		parse_subnet "$subnet_line"
		return
	fi
	
	# Otherwise, extract from Address line in [Interface] section
	local address_line=$(grep "^Address = " "$WG_CONF" | head -1 | cut -d '=' -f 2 | tr -d ' ' | cut -d ',' -f 1)
	if [ -z "$address_line" ]; then
		exiterr "Could not find Address in WireGuard configuration file."
	fi
	
	# Extract IP and prefix length (e.g., "10.7.0.1/24" -> "10.7.0.0/24")
	local gateway_ip=$(echo "$address_line" | cut -d '/' -f 1)
	local netmask=$(echo "$address_line" | cut -d '/' -f 2)
	
	# Extract network base (first three octets)
	local base_ip=$(echo "$gateway_ip" | cut -d '.' -f 1-3)
	local network="${base_ip}.0/${netmask}"
	
	parse_subnet "$network"
}
...
check_args() {
	...
	if [ -n "$server_addr" ] || [ -n "$server_port" ] || [ -n "$first_client_name" ] || [ -n "$vpn_address" ]; then
			if [ -e "$WG_CONF" ]; then
	...
	# Validate and initialize VPN subnet address
	if [ -e "$WG_CONF" ]; then
		# If WireGuard is already installed, read subnet from config
		read_subnet_from_conf
	else
		# If installing, use provided address or default
		if [ -z "$vpn_address" ]; then
			vpn_address="10.8.0.0/24"
		fi
		if ! check_cidr "$vpn_address"; then
			exiterr "Invalid VPN subnet address. Must be in CIDR notation (e.g., 10.8.0.0/24)."
		fi
		parse_subnet "$vpn_address"
	fi
}
...
Options:
  ...
  --dns1 [DNS server IP]         primary DNS server for new client (optional, default: 223.5.5.5)
...
Install options (optional):
  ...
  --address [CIDR]               VPN subnet address in CIDR notation (default: 10.8.0.0/24)
  --clientname [client name]     name for the first WireGuard client (default: client)
  --dns1 [DNS server IP]         primary DNS server for first client (default: 223.5.5.5)
...
show_config() {
	if [ "$auto" != 0 ]; then
		...
		[ -n "$vpn_address" ] && subnet_text="$vpn_address" || subnet_text="10.8.0.0/24"
		...
		if [ -n "$dns1" ] && [ -n "$dns2" ]; then
			...
			dns_text="223.5.5.5, 119.29.29.29"
		fi
		echo "Port: UDP/$port_text"
		echo "VPN subnet: $subnet_text"
...
[Interface]
Address = $vpn_gateway_ip/24$([[ -n "$ip6" ]] && echo ", fddd:2c4:2c4:2c4::1/64")
...
remove_firewall_rules() {
	...
	# Read subnet from config if not already set
...
get_export_dir() {
	export_dir="/etc/wireguard/client/"
...
select_client_ip() {
	# Ensure subnet variables are set
	if [ -z "$vpn_base_ip" ]; then
		if [ -f "$WG_CONF" ]; then
			read_subnet_from_conf
		else
			exiterr "Cannot select client IP: VPN subnet not configured."
		fi
	fi
	# Given a list of the assigned internal IPv4 addresses, obtain the lowest still
	# available octet. Important to start looking at 2, because 1 is our gateway.
	octet=2
	# Escape dots in base IP for grep
	base_ip_escaped=$(echo "$vpn_base_ip" | sed 's/\./\\./g')
	while grep AllowedIPs "$WG_CONF" | grep "^AllowedIPs = $base_ip_escaped\\." | cut -d "." -f 4 | cut -d "/" -f 1 | grep -q "^$octet$"; do
		(( octet++ ))
	done
...
new_client() {
	...
	if [ -z "$vpn_base_ip" ]; then
		if [ -f "$WG_CONF" ]; then
			...
			exiterr "Cannot create client: VPN subnet not configured."
		fi
	fi
	select_client_ip
	specify_ip=n
	if [ "$1" = "add_client" ] && [ "$add_client" = 0 ]; then
		...
		if [[ ! "$specify_ip" =~ ^[yY]$ ]]; then
			echo "Using auto assigned IP address $vpn_base_ip.$octet."
		fi
	fi
	if [[ "$specify_ip" =~ ^[yY]$ ]]; then
		echo
		base_ip_escaped=$(echo "$vpn_base_ip" | sed 's/\./\\./g')
		read -rp "Enter IP address for the new client (e.g. $vpn_base_ip.X): " client_ip
		octet=$(printf '%s' "$client_ip" | cut -d "." -f 4)
		ip_pattern="^${base_ip_escaped}\\.([2-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-4])$"
		base_ip_escaped_grep=$(echo "$vpn_base_ip" | sed 's/\./\\./g')
		until [[ $client_ip =~ $ip_pattern ]] \
			&& ! grep AllowedIPs "$WG_CONF" | grep "^AllowedIPs = $base_ip_escaped_grep\\." | cut -d "." -f 4 | cut -d "/" -f 1 | grep -q "^$octet$"; do
			if [[ ! $client_ip =~ $ip_pattern ]]; then
				echo "Invalid IP address. Must be within the range $vpn_base_ip.2 to $vpn_base_ip.254."
			else
...
PresharedKey = $psk
AllowedIPs = $vpn_base_ip.$octet/32$(grep -q 'fddd:2c4:2c4:2c4::1' "$WG_CONF" && echo ", fddd:2c4:2c4:2c4::$octet/128")
# END_PEER $client
EOF
...
[Interface]
Address = $vpn_base_ip.$octet/24$(grep -q 'fddd:2c4:2c4:2c4::1' "$WG_CONF" && echo ", fddd:2c4:2c4:2c4::$octet/64")
DNS = $dns
PrivateKey = $key
